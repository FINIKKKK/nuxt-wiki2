...
const tabsParse = computed(() => {
  return article.value?.tabs.map((obj) => ({
    name: obj.name,
    content: JSON.parse(obj.content),
  }));
});
const select = computed(() => {
  return sections.value.find((obj) => obj.id === article.value.section_id);
});

/**
 * Пользовательские переменные ----------------
 */
const titleValue = ref(article.value?.name || ''); // Заголовок элемента
const selectValue = ref<TSection | null>(select.value || null); // Селект элемента
const tabs = ref(tabsParse || []);
...
const onSubmit = async () => {
  ...
  if (isEdit) {
    // const tabs1 = tabs.value?.map((obj) => ({
    //   name: obj.name,
    //   content: JSON.parse(obj.content),
    // }));
    //
    ...
    const dto = {
      ...
      tabs: tabs.value?.map((obj) => ({
        name: obj.name,
        content: JSON.stringify(obj.content),
      })),
      section_id: String(selectValue.value?.id),
      action: 3,
    };
    console.log(dto);
    ...
    validateForm(dto, SectionScheme, async () => {
      // Изменяем раздел
      const { data } = await Api().article.edit(dto);
      console.log(data);
